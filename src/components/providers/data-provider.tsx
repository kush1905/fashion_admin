...
import { api, getApiBaseUrl } from "@/services/api/client";
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  if (status === "error") {
    let configuredBase = "(unresolved)";
    try {
      configuredBase = getApiBaseUrl();
    } catch (err) {
      configuredBase = err instanceof Error ? err.message : "NEXT_PUBLIC_API_URL is missing";
    }
    return (
      <div className="flex min-h-dvh items-center justify-center bg-background p-6">
        <div className="max-w-md space-y-3 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            The admin could not reach the atelier API. Check that the backend is reachable and that{" "}
            <code className="text-xs">NEXT_PUBLIC_API_URL</code> is set for this deployment.
          </p>
          <p className="break-all font-mono text-[11px] text-muted-foreground">API base: {configuredBase}</p>
          {error ? <p className="break-all text-xs text-muted-foreground">{error}</p> : null}
